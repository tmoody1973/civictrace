...
| **2:10–2:35** | Approval drawer in the review pane: the staged question, its **sha256 fingerprint**, and the expiry rule (`proof/moo-705-drawer-hash-expiry.png`). Type the reviewer name → Approve → **DRAFT packet** appears with banner + its own hash (`proof/moo-705-packet-view.png`, `proof/moo-706-packet-view.png`). Then the money shot: a tampered approval → on-screen refusal “you approved different bytes than are staged”, `APPROVAL_REFUSED` row in the trace, no file (`proof/moo-705-refusal-tampered-hash.png`, `proof/moo-706-refusal-state.png`). | “The editor approves the exact bytes — the fingerprint on screen. Change one word and the approval dies with it: the system refuses in words, in the ledger, and renders nothing. Authority over anything external stays human.” | Full workflow / scoped action / fail closed. |
...
| **3:15–3:40** | Google Cloud Console, in this exact order (framing proven in `docs/hackathon/proof/moo-710-*.jpg`): (1) Cloud Tasks queue `civictrace-ingest` (healthy, drained), (2) Firestore `cases/case-tid121…/ledger_events` (the durable ledger, seq 1–17+), (3) Cloud Storage vault bucket (3 immutable record PDFs, Not public), (4) BigQuery Job history → Project history (the worker service account's prefilter queries), (5) worker logs at the replay window, (6) Billing budget page (`civictrace-dev-10usd`). Then architecture diagram. | “This is deployed on Google Cloud. Here is the live service, durable state, event infrastructure, and trace for the exact case you just saw — inside a ten-dollar budget.” | Production readiness. |
